...
def publish_post(
    ...
    attachment_folder: Annotated[
        ...
        typer.Option(
            "--attachment-folder",
            "-a",
            help="Subfolder in vault to search for images (repeatable)",
        ),
    ] = None,
    slug: Annotated[
    ...
    no_llm: Annotated[
        bool, typer.Option("--no-llm", help="Skip LLM calls. Implies --dry-run.")
    ] = False,
    verbose: Annotated[
        bool, typer.Option("--verbose", "-V", help="Show extra debug output.")
    ] = False,
    commit: Annotated[
        bool, typer.Option("--commit", help="Git commit after writing")
    ] = False,
    auto_alt: Annotated[
        bool,
        typer.Option(
            "--auto-alt",
            help="Automatically generate alt text for images using vision LLM",
        ),
    ] = False,
    vision_model: Annotated[
        str,
        typer.Option(
            "--vision-model", help="Vision model to use for alt text generation"
        ),
    ] = "@vision",
    ...
    if not hugo_dir:
        typer.secho(
            "Error: Hugo site directory not specified. Use --hugo-dir or set BLOG_PATH.",
            fg=typer.colors.RED,
        )
        raise typer.Exit(1)
...
def publish_find(
    ...
    target_dir = handle_find(
        input_file, hugo_dir, dry_run=dry_run, no_llm=no_llm, verbose=verbose
    )
